...
//It provides better performance by reducing overall seek time. It also accept direction of head movement.
#include<stdio.h>
#include<stdlib.h>
...
void scan(int req[], int n, int head,int maxc,int  d)
{
	...
	hm = abs(0 - head);
        total = total + hm;
        head=0;
        printf("Reached to Left End, Head Movement : %d\n",hm);
	for (i = index; i < n; i++) {
            hm = abs(req[i] - head);
            printf("Request %d served. Head movement: %d\n", req[i], hm,total);
            total = total + hm;
...
void main()
{
	...
	scanf("%d",&maxc);
	printf("\nEnter Direction of Head (1 for Left / 2 for Right) : ");
	scanf("%d",&direction);
	printf("\nSeek Sequence : %d",head);

	scan(req,n,head,maxc,direction);
}
